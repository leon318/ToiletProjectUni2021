def find_shortest_length(filenames, original_length_listxy, original_length_listxz, x_listy, x_listz):
    """
    ...
    for idx, filename in enumerate(filenames): #This prints the length of both the x and y files. This will allow us to cut away from the end so they
        original_length_listxy.append(len(x_listy[idx])) #This puts the length of each x file into another list. Its the same for the y.
        original_length_listxz.append(len(x_listz[idx]))
    d = min(original_length_listxy) #This defines
    g = min(original_length_listxz)
    print(d)
    print(g)
    return d,g
